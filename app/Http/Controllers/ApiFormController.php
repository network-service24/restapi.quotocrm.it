    ...
    public function from_id_to_cod($value)
    {
        ...
        public function insert_preventivo(Request $request)
        {
            ...
            if ($permessi > 0) {
                ...
                if($request->captcha == 1){
                    /** controllo captcha se attivo */
                            $check_captcha = 'g-recaptcha-response';
                            if (isset($request->$check_captcha)) {
        
                                $secret_key = $chiave_segreta_recaptcha;
                                $url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . $secret_key . '&response=' . $request->$check_captcha;
                                ...
                                if ($responseCaptchaData->success) {
                                        ...
                                        if ($nome != '' && $cognome != '' && $email != '' && $urlback != '' && $language != '' && $adulti != '' && $arrivo != '' && $partenza != '') {
                                            ...
                                            DB::insert('hospitality_guest', $data);
         ...
         public function send_form(Request $request)
        {
            $data = [
                    ...
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, 'http://127.0.0.1:8000/api/insert_preventivo');
        ...
        */
        public function send_form(Request $request)
        {
            ...
            $response = Http::asForm()->post(
                'http://127.0.0.1:8000/api/insert_preventivo', [
                    'oggetto_email'   => 'Richiesta per https://test.suiteweb.it',
                    'idsito'          => 1740,
                    'id_lingua'       => 1,
                    'language'        => 'it',
